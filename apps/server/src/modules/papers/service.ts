...
export function toPaperResponse(paper: Paper): PaperResponseType {
	return {
		id: paper.id,
		title: paper.title,
...
export function buildFacets(papersList: FacetRow[]): FacetsType {
...
export function buildOrderBy(sortBy: SortByType) {
	switch (sortBy) {
		case "date_desc":
...
export function searchCacheKey(params: object): string {
...
export async function mlSearchPapers(
	...
	const safePageSize = [10, 20, 50].includes(params.pageSize ?? 20)
		? (params.pageSize ?? 20)
		: 20;
	...
	const fieldFilter =
		params.field && isFieldOfStudy(params.field)
			? fieldCondition(params.field)
			: undefined;
	...
	if (params.sortBy && params.sortBy !== "relevance") {
		...
		if (orderBy) {
			...
			filteredRows.sort((a, b) => {
				switch (params.sortBy) {
					case "date_desc":
						return (
							(b.published_at?.getTime() ?? 0) -
							(a.published_at?.getTime() ?? 0)
						);
					case "date_asc":
						return (
							(a.published_at?.getTime() ?? 0) -
		...
		filteredRows.sort(
			(a, b) =>
				(rankMap.get(a.id) ?? Number.POSITIVE_INFINITY) -
				(rankMap.get(b.id) ?? Number.POSITIVE_INFINITY)
		);
	}
